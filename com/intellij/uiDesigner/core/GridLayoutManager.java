  ...
  extends AbstractLayout
{
  ...
  public GridLayoutManager(int rowCount, int columnCount)
  {
    ...
    for (int i = 0; i < rowCount; i++) this.myRowStretches[i] = 1;

    this.myColumnStretches = new int[columnCount];
    for (int i = 0; i < columnCount; i++) this.myColumnStretches[i] = 1;
  ...
  public void addLayoutComponent(Component comp, Object constraints)
  {
    ...
    if ((column < 0) || (column >= columnCount)) throw new IllegalArgumentException("wrong column: " + column);

    if (column + colSpan - 1 >= columnCount) throw new IllegalArgumentException("wrong col span: " + colSpan + "; column=" + column + " columnCount=" + columnCount);
  ...
  private static void makeSameSizes(int[] widths)
  {
    ...
    for (int i = 0; i < widths.length; i++) widths[i] = max;
  ...
  public Dimension preferredLayoutSize(Container container)
  {
    ...
    if (this.mySameSizeVertically) makeSameSizes(heights);
  ...
  private static int getDesignTimeInsets(Container container)
  {
    while (container != null)
    {
      if ((container instanceof JComponent))
      {
        ...
        if (designTimeInsets != null) return designTimeInsets.intValue();
  ...
  private static int countGap(DimensionInfo info, int startCell, int cellCount)
  {
    ...
    for (int cellIndex = startCell + cellCount - 2; cellIndex >= startCell; cellIndex--) {
      shouldAddGapAfterCell(info, cellIndex)&& counter++;
    }
  ...
  {
    if ((cellIndex < 0) || (cellIndex >= info.getCellCount())) throw new IllegalArgumentException("wrong cellIndex: " + cellIndex + "; cellCount=" + info.getCellCount());
    ...
    for (int i = 0; i < info.getComponentCount(); i++)
    {
      ...
      {
        if ((info.componentBelongsCell(i, cellIndex)) &&   (DimensionInfo.findAlignedChild(component, info.getConstraints(i)) != null)) return true;

        info.getCell(i) == indexOfNextNotEmpty && startsInNext = true;

        info.getCell(i) + info.getSpan(i) - 1 == cellIndex && endsInThis = true;
